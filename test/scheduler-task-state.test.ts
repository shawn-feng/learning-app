...
describe("getChildState 向前兼容老结构", () => {
  it("老结构（仅 recording）缺失后续键时不崩溃且补齐该键", () => {
    const state: TaskState = {
      children: {
        // 模拟升级前已存在于磁盘上的孩子状态，没有 auto-new-session/event-poll/class-reminder/todo 键
        "kid-1": {
    ...
    // ISSUE-025：todolist 键
    expect(cs.todo).toBeDefined();
    expect(cs.todo.lastRun).toBe("");
  ...
  it("全新孩子初始化出完整五键结构", () => {
    ...
    expect(Object.keys(cs).sort()).toEqual(
      ["auto-new-session", "class-reminder", "event-poll", "recording", "todo"].sort()
    );
