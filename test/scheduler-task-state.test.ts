...
describe("getChildState 向前兼容老结构", () => {
  it("老结构（仅 recording）缺失 auto-new-session/event-poll 时不崩溃且补齐该键", () => {
    const state: TaskState = {
      children: {
        // 模拟升级前已存在于磁盘上的孩子状态，没有 auto-new-session/event-poll 键
        "kid-1": {
    ...
    // 关键：不抛错，且 auto-new-session/event-poll 被补齐为合法对象
