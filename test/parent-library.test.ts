...
describe("ISSUE-029 家长库（主题统一管理 + 快照分配，SPLIT 服务端模式）", () => {
  ...
  it("allocateTopicToChild 快照拷贝主题进孩子库；重复分配不丢孩子进度", async () => {
    ...
    // 验证孩子库课程进度仍在；2026-09-18 库域分工后孩子库 courses 只剩进度域字段
    //（mastery 早已下线；material/lesson_method/html_path 等教学内容真源在家长库，经 kb.courses.get 实时读）
    ...
    expect(c.topic_key).toBe("lunyu"); // topic_key 真引用已回填
    expect(c.mastery).toBeUndefined();
    expect(c.lesson_method).toBeUndefined();
    expect(c.html_path).toBeUndefined();
    expect(c.teaching_copy).toBeUndefined();
    // 用户拍板（2026-09-04）：主题级教学方法**不快照**进孩子库——2026-09-18 起该列已删，
    // 教法真源始终在家长库，孩子端经服务端 kb.courses.get / parent_content 实时读家长库。
    const kt = await dbQuery<any[]>("kb.topics.list", { child_id: CHILD });
    expect(kt.find((m: any) => m.name === "论语")?.method).toBeUndefined();
  });
