...
- **麦克风权限：app 不在「系统设置→隐私与安全性→麦克风」列表、从不弹授权窗** → 根因是 app 的 `Info.plist` 缺 `NSMicrophoneUsageDescription`，macOS（Ventura+）会**静默拒绝且根本不把 app 登记进权限列表**；且原 `main.ts` 只在 Chromium 层 `setPermissionRequestHandler` 放行、没有系统级「主动申请」。修复（已固化）：① `package.json` `build.mac.extendInfo` 加 `NSMicrophoneUsageDescription` / `NSSpeechRecognitionUsageDescription` 两条声明；② `electron/main.ts` `app.whenReady` 内新增 `if (process.platform === "darwin") systemPreferences.askForMediaAccess("microphone")`，使 app 启动即出现在权限列表并主动弹窗。CI 的 `Verify Info.plist microphone permission declarations` 步骤已用 `PlistBuddy` 打印主 app 的两条声明做铁证（注意 find 必须 `! -path "*/Frameworks/*"` 排除 Helper app，否则会误命中 `学习伙伴 Helper (GPU).app` 的 plist 而误报）。若用户**之前已拒绝过**授权，`askForMediaAccess` 不再弹窗，需手动去系统设置开启。
