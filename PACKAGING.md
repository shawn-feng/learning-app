...
- `package.json`：新增脚本 `"dist:mac": "electron-vite build && electron-builder --mac"`；`build.mac` 补 `"category": "Education"`，`target` 保持 `["dmg"]`。**不写 `identity`** → 无 Developer ID 证书时 electron-builder 自动 fallback 为 ad-hoc 签名（零成本，用户下载后手动放行）。
