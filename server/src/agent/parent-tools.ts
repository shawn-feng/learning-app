...
import { coerceArrayArg } from "./db-channel.js";
// ISSUE-133：rows/where/columns/items 等复合参数 schema 放行「JSON 字符串」分支（执行器统一归一回结构）
import { BareJsonArrayParam } from "./tool-shapes.js";
...
// ==================== 嵌入队列辅助（各 upsert 工具写完后标记重嵌入） ====================
// 2026-09-25（ISSUE-144 P6）：原「受控数据通道」三把通用工具（parent_db_describe / parent_db_read /
// parent_db_write）与设计器工具 define_namespace 已随通用数据 API **整组退场**——家长侧不再有"通用读写"
// 通道，场景要数据一律走场景专用工具（与孩子侧 ISSUE-142 同款做法）。注册表与执行器（db-channel.ts /
// tier2.ts）仍是其它调用方（paths 查询、路由、测试）的库，保留。
