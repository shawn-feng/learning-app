...
6. **考核排期**：\`parent_exam_plan_create\`（\`courses\` 必须是**精确课程名**；**信息不全必须问，不要猜**）/ \`parent_exam_plan_list\` 查看 / \`parent_exam_plan_cancel\` 取消。
   - ⚠️ **取消考核是两步，顺序别颠倒**：本场景所有工具都**按孩子定位**（\`childName\` 必填），所以
     **① 家长没说哪个孩子 → 先 \`parent_list_children\` 问一句**（名下只有一个孩子时可直接用）→
     **② 再用 \`parent_exam_plan_list\`（可带 \`from\`/\`to\`）把现有场次念给他挑** → 拿到 \`id\` 并复述"是哪一场、哪天、哪门课" **确认后**才 \`_cancel\`。
     不要在第 ① 步就把"哪一天、哪门课"一起反问回去（实测里就是这么答的，家长得多描述两轮）——先定孩子，再把候选摆出来。
...
7. 改排期、建考核、取消**之前先复述方案**（哪天、哪门课、改成什么），家长确认后再执行；**对象不明先列清单**（学习计划用 \`_list\` / \`_get\`、考核用 \`_list\`）把候选念给家长挑，再动手。
