...
- 201 服务端运行方式：`/usr/bin/node /opt/learning-server/server.cjs`（systemd `learning-server`，以 root 跑，pkg 已弃用）；数据目录 `/opt/learning-server/data`；健康端点 `/api/v1/health`。部署＝stop → 备份（bundle→`server.cjs.bak-<ts>`，数据→`data/backups/deploy-<ver>-<ts>/`）→ 换 bundle → daemon-reload + restart，停机约 6 秒。**当前已部署 0.5.7（2026-09-23 11:39，含 0.5.6 + 修复）：0.5.6 = ISSUE-135 P0-a（考核结果直写孩子库三层 `exam_plan_courses`/`exam_course_results`/`knowledge_point_records` + `speech_assessments` 落孩子库 + 主库 `exam_attempts`/`speech_assessments` DROP + 读取侧全切孩子库 + `courses` 掌握四列 + `study_plans.result_summary` + 计划防硬删）+ P4（默认「学习情况分析」自定义任务每天 21:30 + 4 个 `mastery_*` 工具）；**0.5.7 = 修复 0.5.6 的线上 500**（`GET /exam/attempts/:childId` 误 select `exam_plans.topic_key`，该表无此列）+ 新增路由级冒烟测试 `test/issue135-exam-routes.test.ts`。部署验证口径：`/api/v1/version` 版本号 + bundle 标记 + 新路由 401（不是 404）+ `journalctl` ERR_COUNT=0 + **自签家长 JWT 直连 127.0.0.1:8788 跑端到端读接口**（`data/server-config.json` 有 `jwtSecret`，纯 Node `crypto` HS256 即可，模板 `tmp/deploy/verify_056_api.js`）。** 备份：bundle `server.cjs.bak-20260923-1205`（0.5.8）/`server.cjs.bak-20260923-1138`（0.5.7）、数据 `data/backups/deploy-0.5.7-20260923-1138/`（另 `deploy-0.5.6-20260923-1128/` 保留了**DROP 前**的 19 行 `exam_attempts` 快照）。
