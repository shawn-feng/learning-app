...
- 201 服务端运行方式：`/usr/bin/node /opt/learning-server/server.cjs`（systemd `learning-server`，以 root 跑，pkg 已弃用）；数据目录 `/opt/learning-server/data`；健康端点 `/api/v1/health`。部署＝stop → 备份（bundle→`server.cjs.bak-<ts>`，数据→`data/backups/deploy-<ver>-<ts>/`）→ 换 bundle → daemon-reload + restart，停机约 6 秒。**当前已部署 0.5.10（2026-09-24 15:53）+ web 前端包同步换装（同为 15:53）：0.5.8 = ISSUE-135 P0-a/P4 掌握闭环（0.5.6+0.5.7 修复线上 500）+ 0.5.8 掌握分析任务改显式添加；0.5.9 = ISSUE-143 发音评测音频双根解析修复**（`readAudioBytes` P2 归并后只查旧根 → 0.5.5 起评测全 404「音频文件不存在」；改用 `resolveStoredFileAbs` + 失败留痕 warn；随包 ISSUE-142 孩子侧专用报告工具 + ISSUE-144 家长场景 skill P0~P4+P7）；**0.5.10 = ISSUE-146 P0**（240s 挂死看门狗判据分流：有工具在跑走 30min 硬上限 + 工具超时文案，否则原 240s 模型静默；新增 `server/src/agent/session-activity.ts`；编程工具接 `signal`/`onUpdate` → `tool_progress` 进度心跳；顺带修孩子侧 `activityKey≠streamKey` 使任何 >240s 正常轮都被砍的独立 bug）。
⚠ **201 的 web 前端是独立部署项**（`/opt/learning-server/web/dist`，非随 bundle）：2026-09-24 15:53 之前**一直停在 09-22 07:46 的 `index-BiRAcmS8.js`**，即 ISSUE-145 的修复（45s 心跳看门狗 + `/parent-agent/open` 历史回填）**在浏览器里从未生效过**；现为 `index-BSngxzoI.js`（md5 `6e1f8266ee27dfa41cfe5589579dd85d`）。**以后凡改 `web/src/**` 都必须单独跑一次 web swap**（模板 `tmp/deploy/web_deploy_0510.py` + `web_swap_0510.sh`）。
部署验证口径：`/api/v1/version` 版本号 + bundle 标记 + `journalctl` ERR_COUNT=0 + 自签家长 JWT 直连 127.0.0.1:8788 跑端到端（`data/server-config.json` 有 `jwtSecret`，纯 Node `crypto` HS256，模板 `tmp/deploy/verify_0510_api.js`；⚠ 考核明细/files 表 `created_at` 是 **UTC** ISO，按本地日期 LIKE 会漏行）。** 备份：bundle `server.cjs.bak-20260924-1600`（0.5.9）、数据 `data/backups/deploy-0.5.10-20260924-1600/`、旧 web 包 `web/dist.old-20260924-1600`；更早 `server.cjs.bak-20260924-1116`（0.5.8）等。**未验证**：ISSUE-146 的真实长工具轮（需一次 >240s 的网页生成才能实证不再被砍）；浏览器端到端刷新可见历史；Electron 桌面端（本机 + 201 的 xuexihub 0.1.15）仍需 `npm run dist:win` / 重打 deb。
...
- ⚠️ **201 磁盘（2026-09-24 两档清理后实测）**：根分区 116G 用 104G＝**95%，仅剩 6.0G**。**09-24 清理**（用户指令）：①删一周前部署备份（< 09-17：bundle bak 19 个 304M + deploy 快照 4 个 150M）；②09-17 后按「保留最新 5 个」收紧（bundle bak 删 16 个 357M + deploy 快照删 14 个约 500M）+ 删 `learning-server.pkg030`（83M）——**当日累计约 1.4G**。**现留回滚阶梯**：bundle bak 5 个（0.5.5/0.5.6/0.5.7/0.5.8/0.5.9 前夜）+ deploy 快照 5 个（0.5.4~0.5.7 + 0.5.9，**deploy-0.5.6 内含 DROP 前 exam_attempts 快照**）。仍留未删（非部署备份，需逐项确认）：09-11 操作前快照×4、lunyu-sync-20260920、08-30 孩子数据备份、yunlvcao 原始 mp4 8.4M。以后每次部署顺手照「保留最新 5 个」清。
